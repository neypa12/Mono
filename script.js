    ...
    async function performTransaction(senderId, recipientId, amount) {
        ...
        try {
            await db.runTransaction(async (transaction) => {
                ...
                // Registrar Transacción (CORREGIDO: Uso de serverTimestamp)
            ...
            if (e === "Saldo insuficiente.") {
                 ...
                 // Aquí se muestra el error de Firebase
    ...
    function startAppListeners() {
        ...
        db.collection("transactions").orderBy("timestamp", "desc").onSnapshot((snapshot) => {
            ...
            snapshot.forEach((doc) => {
                ...
                // Manejo seguro del timestamp (para evitar errores si el campo es corrupto o nulo)
                let time = '...';
                const timestampValue = data.timestamp;
                if (timestampValue && typeof timestampValue.toDate === 'function') {
                    try {
                        time = new Date(timestampValue.toDate()).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit'});
                    } catch (e) {
                        time = 'Fecha inválida'; 
                    }
                }
